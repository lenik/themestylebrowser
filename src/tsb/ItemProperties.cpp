...
ItemPropertiesDialog::ItemPropertiesDialog(wxWindow* parent, const wxString& title,
    ...
    wxString line2 = wxString::Format(tr("%d styles in %zu formats: %s"), item->count,
                                      item->formats.size(), fmtStr.c_str());
    wxString line3;
    if (item->total_bytes >= 1024 * 1024)
        line3 = wxString::Format(tr("%.1f M total"), item->total_bytes / (1024.0 * 1024.0));
    else if (item->total_bytes >= 1024)
        line3 = wxString::Format(tr("%.1f k total"), item->total_bytes / 1024.0);
    else
        line3 = wxString::Format(tr("%lld total"), (long long)item->total_bytes);
    ...
    top->Add(new wxStaticText(this, wxID_ANY, tr("Path Info:")), 0, wxALL, 2);
    ...
    wxButton* copyPath = new wxButton(this, wxID_ANY, tr("Copy"));
    pathRow->Add(copyPath, 0, wxALL, 2);
    top->Add(pathRow, 0, wxEXPAND | wxALL, 5);
    top->Add(new wxStaticLine(this), 0, wxEXPAND | wxALL, 5);
    top->Add(new wxStaticText(this, wxID_ANY, tr("Code Snippets:")), 0, wxALL, 2);
    wxChoice* toolChoice = new wxChoice(this, wxID_ANY);
    toolChoice->Append(tr("bas-ui icon ImageSet"));
    ...
    wxButton* copySnippet = new wxButton(this, wxID_ANY, tr("Copy"));
    top->Add(copySnippet, 0, wxALL, 5);
    top->Add(new wxStaticLine(this), 0, wxEXPAND | wxALL, 5);
    wxBoxSizer* btnRow = new wxBoxSizer(wxHORIZONTAL);
    btnRow->Add(new wxButton(this, wxID_OK, tr("OK")), 0, wxALL, 5);
    btnRow->Add(new wxButton(this, wxID_CLOSE, tr("Close")), 0, wxALL, 5);
