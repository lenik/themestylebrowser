...
#
# Environment:
#   THEMESTYLEBROWSER_SRCDIR       project root (default: packaging/../..)
#   THEMESTYLEBROWSER_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
THEMESTYLEBROWSER_SRCDIR=${THEMESTYLEBROWSER_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$THEMESTYLEBROWSER_SRCDIR/.config/themestylebrowser" "$HOME/.config/themestylebrowser"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${THEMESTYLEBROWSER_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${THEMESTYLEBROWSER_PACKAGING:-}")"
...
cmd_run() {
    ...
    if [[ ${THEMESTYLEBROWSER_FORCE_LOCAL:-} == 1 ]]; then
        exec "$@"
    fi
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $THEMESTYLEBROWSER_SRCDIR/.config/themestylebrowser/$k.build-host" >&2
        echo "    $HOME/.config/themestylebrowser/$k.build-host" >&2
    done
    return 2
}
